...
import type { HouseholdImportIssue } from '../../api/householdApi'
import Icon from '../Icon'
import type { ImportPreviewModalProps } from './ImportPreviewModal.type'
...
export default function ImportPreviewModal({ preview, confirming, error, onClose, onConfirm }: ImportPreviewModalProps) {
  ...
  return <dialog
    ...
    className="fixed left-1/2 top-[47.5%] right-auto bottom-auto m-0 -translate-x-1/2 -translate-y-1/2 max-h-[calc(100dvh-1.5rem)] w-[calc(100%-1.5rem)] max-w-lg overflow-y-auto rounded-3xl border-0 bg-white p-0 text-ink shadow-2xl backdrop:bg-ink/50 backdrop:backdrop-blur-sm sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100%-2rem)]"
    onCancel={event => { event.preventDefault(); close() }}
    onClick={event => { if (event.target === event.currentTarget) close() }}
  >
    <div className="p-5 sm:p-7">
      <div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-sage text-pine" aria-hidden="true">
        <Icon name="upload" className="h-5 w-5" />
      </div>
      <h2 id={titleId} className="mt-5 text-center text-2xl">Review import</h2>
      <p id={descriptionId} className="mt-2 text-center leading-relaxed text-stone-600">
        Review what will be added to {preview.destinationHouseholdName} from {sourceName} before importing.
      </p>

      <div className="mt-5 rounded-2xl bg-cream px-4 py-3 text-center text-sm leading-relaxed text-stone-600">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
        {summaries.map(([label, count]) => <div key={label} className="rounded-2xl bg-cream px-3 py-3 text-center">
          <p className="text-xs font-semibold text-stone-500">{label}</p>
          <p className="mt-1 text-xl font-bold text-pine">{count}</p>
        </div>)}
      </div>

      <div className="mt-5 space-y-3 text-center text-sm leading-relaxed text-stone-600">
      ...
      {preview.errors.length > 0 && <div className="mt-5 rounded-2xl bg-red-50 px-4 py-3 text-center text-sm text-red-700">
        <p className="font-bold">Fix these issues before importing:</p>
        <ul className="mx-auto mt-2 inline-block list-disc space-y-1 pl-5 text-left">
          {preview.errors.map((issue, index) => <li key={`${issue.path}-${index}`}>{issueLabel(issue)}</li>)}
        </ul>
      </div>}

      {preview.warnings.length > 0 && <div className="mt-5 rounded-2xl bg-amber-50 px-4 py-3 text-center text-sm text-amber-800">
      ...
      {error && <p id={errorId} role="alert" className="mt-4 rounded-xl bg-red-50 px-4 py-3 text-center text-sm text-red-700">{error}</p>}
