...
import { getSpaceColor, withColorAlpha } from '../spaceColors'
  ...
  return <>
    ...
    <section className="mt-10">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {data.rooms.map((room, index) => { const color = getSpaceColor(room.color, index); return <Link to={`/items?roomId=${room.id}`} className="card group overflow-hidden p-0 transition hover:-translate-y-0.5 hover:shadow-soft" key={room.id}>
