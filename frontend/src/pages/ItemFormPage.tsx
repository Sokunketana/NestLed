...
export default function ItemFormPage() {
  ...
  return <>
    ...
    <div className="mt-5 max-w-3xl"><p className="eyebrow">{editing ? 'Update record' : 'New record'}</p><h1 className="mt-2 text-4xl">{editing ? 'Edit item' : 'Add an item'}</h1><p className="mt-2 text-stone-500">Record what it is, what it is worth, and exactly where it lives.</p></div>
    ...
    <form onSubmit={submit} className="mt-8 space-y-6">
      ...
      <section className="card grid gap-5 md:grid-cols-2"><div className="mb-1 flex items-center gap-3 md:col-span-2"><span className="grid h-9 w-9 place-items-center rounded-xl bg-sage text-pine"><Icon name="box" className="h-4 w-4" /></span><div><h2 className="text-xl">The basics</h2><p className="text-sm text-ink-soft">Start with the details you’ll use to recognize it.</p></div></div><div className="md:col-span-2"><label className="label">Item name *</label><input className="field" required maxLength={150} value={form.name} onChange={e => set('name', e.target.value)} placeholder="e.g. Passport" /></div>
      ...
      <section className="card">
        <div className="flex items-center gap-3"><span className="grid h-9 w-9 place-items-center rounded-xl bg-coral/10 text-coral"><Icon name="camera" className="h-4 w-4" /></span><div><h2 className="text-xl">Item photo</h2><p id="photo-help" className="mt-1 text-sm text-stone-500">Optional · JPG, PNG, WebP, or GIF · 5 MB maximum</p></div></div>
      ...
      <div className="flex justify-end gap-3"><Link to={editing ? `/items/${id}` : '/items'} className="btn-secondary">Cancel</Link><button className="btn-primary" disabled={saving || !rooms.length || !categories.length || !form.storageLocationId}><Icon name={saving ? 'history' : editing ? 'check' : 'plus'} className="h-4 w-4" />{saving ? (photoFile ? 'Uploading photo…' : 'Saving…') : editing ? 'Save changes' : 'Add item'}</button></div>
    </form>
  </>
}
