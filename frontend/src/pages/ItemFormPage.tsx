...
export default function ItemFormPage() {
  ...
  return <>
    ...
    <form onSubmit={submit} className="mt-8 space-y-6">
      ...
      <div className="flex justify-end gap-3"><Link to={editing ? `/items/${id}` : '/items'} className="btn-secondary">Cancel</Link><button className="btn-primary" disabled={saving || !rooms.length || !categories.length}>{saving ? (photoFile ? 'Uploading photo…' : 'Saving…') : editing ? 'Save changes' : 'Add item'}</button></div>
    </form>
  </>
}
