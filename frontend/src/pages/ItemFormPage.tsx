...
export default function ItemFormPage() {
  ...
  return <>
    ...
    <form onSubmit={submit} className="mt-8 space-y-6">
      ...
      <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Link to={editing ? `/items/${id}` : '/items'} className="btn-secondary w-full sm:w-auto">Cancel</Link><button className="btn-primary w-full sm:w-auto" disabled={saving || !rooms.length || !categories.length || !form.storageLocationId}><Icon name={saving ? 'history' : editing ? 'check' : 'plus'} className="h-4 w-4" />{saving ? (photoFile ? 'Uploading photo…' : 'Saving…') : editing ? 'Save changes' : 'Add item'}</button></div>
    </form>
  </>
}
