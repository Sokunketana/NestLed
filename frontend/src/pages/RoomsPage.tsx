...
export default function RoomsPage() {
  ...
  async function removeRoom(room: Room) { if (confirm(`Delete “${room.name}”? Rooms containing locations or items cannot be deleted.`)) try { await roomApi.remove(room.id); await load() } catch(e) { setError((e as Error).message) } }
  async function removeLocation(location: StorageLocation) { if (confirm(`Delete “${location.name}”? Locations containing items cannot be deleted.`)) try { await storageLocationApi.remove(location.id); await load() } catch(e) { setError((e as Error).message) } }
